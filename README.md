...
- 模擬倉及真實倉顯示最近兩個交易日的損益變動（最新淨值減前一交易日淨值及當日外部資金流），並標示日期及當日回報；假日不作比較基準，缺價或後備資料顯示空值
- 模擬倉／真實倉／SPY 百分比回報比較
- 1M、3M、6M、1Y、ALL 全域時間篩選；個別組合以 daily 最後記錄（包括缺價日）
  作截止，沒有 daily 時依次使用該組合最新持倉報價日、最新交易日，絕不借用今日
  或另一組合日期。ALL 不限制起點，但仍遵守相同截止日；截止日之後的活動
  會在組合資料日期推進後納入
- 所有表格可匯出 CSV
...
- 交易紀錄預設每頁 10 筆，可選 25／50 筆；更改全域期間回到第一頁，
  正常刷新保留頁碼但會限制到有效頁數。Paper／Live 各自保存顯示狀態。
- 貢獻榜預設獲利前 5＋虧損前 5（虧損由最大開始）；零貢獻及未知估值
  不佔摘要名額，可在「查看全部」分頁中找到。整體總貢獻仍按全體計算。
  資產 identity 保留在詳情及選單，摘要減少重複文字。
- 月度表預設顯示全域篩選內最近 12 個月，也可選擇年份；不繞過全域期間。
- 個股交易（最新在前）及未平倉 FIFO 批次各自分頁，切換資產回到第一頁。
- CSV 不受分頁或貢獻摘要限制。月度「最近 12 個月」匯出全域篩選內全部
  月份；選擇指定年份時，只匯出該年與全域期間的交集。
- 本次只改變前端顯示，不需要更新 VPS 或重建快照。
