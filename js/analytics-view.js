...
export function renderAnalytics(container, portfolio, benchmark, range, name) {
  ...
  const rows = model?.rows.map(r=>`<tr data-total="${r.total ?? ''}"><td><button type="button" data-instrument="${e(r.id)}">${e(r.symbol)}<span class="sr-only"> ${e(r.id)}</span></button></td>${cell(r.realized)}${cell(r.unrealized)}${cell(r.income)}${cell(r.total)}${cell(r.fees)}</tr>`).join('');
  ...
  <article class="analytics-card"><h2>月度績效表</h2><p>依所選範圍按月分組，顯示實際涵蓋日期；首尾月份可能未完整。每日 TWR 複利，超額為回報百分點差。缺價或回報缺口顯示 —。</p>${months.length ? table(`${name}-monthly`,['月份','涵蓋日期','組合 TWR','SPY','超額（百分點）'],months.map(r=>`<tr data-month="${e(r.month)}"><th scope="row">${e(r.month)}</th><td>${e(r.start)} 至 ${e(r.end)}</td><td class="numeric ${valueClass(r.result)}">${pct(r.result,{sign:true})}</td><td class="numeric">${pct(r.spy,{sign:true})}</td><td class="numeric ${valueClass(r.excess)}">${r.excess === null ? '—' : number(r.excess*100)}</td></tr>`).join('')) : '<p>未有可用每日績效資料。</p>'}</article>`;
  ...
  mountTable(container.querySelector(`#${name}-contributions`), { scope: range, mode: 'contribution' });
  mountTable(container.querySelector(`#${name}-monthly`), { scope: range, mode: 'monthly' });
}
