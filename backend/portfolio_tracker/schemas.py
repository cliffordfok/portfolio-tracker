...
from .decimal_utils import money, price, shares
...
def _validate_symbol(value: Any) -> None:
    if not isinstance(value, str) or not SYMBOL_RE.fullmatch(value):
        raise ValidationError("symbol must match ^[A-Z][A-Z0-9.-]{0,9}$")
...
def validate_event(
    ...
    *,
    ...
    _require(
        event,
        {"event_id", "portfolio", "occurred_at", "created_at", "action"},
    )
    ...
    if action == "PORTFOLIO_OPEN":
        ...
        if money(event["amount"]) == 0:
            raise ValidationError("cash flow amount cannot be zero")
    elif action == "AMEND":
        ...
        _validate_symbol(event["symbol"])
        if price(event["close"], field="close") <= 0:
